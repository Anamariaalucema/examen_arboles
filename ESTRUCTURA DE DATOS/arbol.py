...
def crear_arbol():
    opcion = int(input("Ingrese cuantos nodos desea: "))
...
def balancear_arbol(raiz):
    if raiz is None:
        return None
    
    nodos = []
    inorder_traversal(raiz, nodos)
    
    return construir_arbol_balanceado(nodos)

def inorder_traversal(raiz, nodos):
    if raiz is not None:
        inorder_traversal(raiz.izquierdo, nodos)
        nodos.append(raiz.valor)
        inorder_traversal(raiz.derecho, nodos)

def construir_arbol_balanceado(nodos):
    if not nodos:
        return None
    
    medio = len(nodos) // 2
    raiz = Nodo(nodos[medio])
    
    raiz.izquierdo = construir_arbol_balanceado(nodos[:medio])
    raiz.derecho = construir_arbol_balanceado(nodos[medio+1:])
...
while True:
    ...
    if opcion == 1:
        ...
        arbol_balanceado = balancear_arbol(arbol)
        print("Árbol balanceado:")
        recorrer_inorder(arbol_balanceado)
    elif opcion == 9:
        break
    else:
        print("Opción no válida")
